# GSMG.IO 5 BTC PUZZLE - FINAL AUTOMATED SOLVER (MULTIPROCESSING-ENABLED)
# This version focuses exclusively on the "from-to" mapping permutation search,
# which is our most robust and promising theory.
...
def check_permutation_chunk(args):
    ...
    """
    final_template, empty_slots, perm_chunk = args
    results = []
...
def solve_with_from_to_mapping(base_words, from_indices, to_indices):
    ...
    with open(CSV_OUTPUT_FILE, 'a', newline='') as csvfile, multiprocessing.Pool(processes=num_processes) as pool:
        ...
        while not solution_found:
            chunk = list(islice(perms_iterator, CHUNK_SIZE))
            if not chunk: break

            process_chunks = [list(islice(iter(chunk), i, len(chunk), num_processes)) for i in range(num_processes)]
            process_args = [(final_template, empty_slots, p_chunk) for p_chunk in process_chunks if p_chunk]
            ...
            print(f"    ...checked {checked_count} permutations...")
