...
# grid coordinates, and deciphers the hidden final command.
...
def decipher_instruction(instruction_key):
    """
    Runs all 25 possible Caesar cipher shifts on the instruction key
    and prints the results. One of these will be the correct command.
    """
    ...
    for shift in range(1, 26):
        ...
        for char in instruction_key:
            if char in alphabet:
                ...
                # Keep non-alphabetic characters (like ':', '.', ' ') as they are
                deciphered_text += char
        
        # ROT is typically named by the decryption shift value
        print(f"Shift +{shift:02d} (ROT-{26-shift:02d}): {deciphered_text}")
    
    print("-" * 40)
    print("[!] ACTION: Inspect the output above to find the human-readable command.")
